...
from preprocess import load_clone_data_by_tag
from models.clone_pp import ClonePPModel
...
def train(args, train_dataset, eval_dataset, model, local_rank):
    ...
    for idx in range(args.epochs):
        ...
        for step, batch in enumerate(bar):
            ...
            loss, prob, _ = model(input_id, input_mask, label)
...
def evaluate(args, eval_dataset, mymodel, local_rank):
    ...
    for batch in eval_dataloader:
        input_id, input_mask, label = [x.to(local_rank) for x in batch]
        with torch.no_grad():
            loss, prob, new_prompt = mymodel(input_id, input_mask, label)
...
def test(args, test_dataset, mymodel, local_rank):
    ...
    for batch in test_dataloader:
        input_id, input_mask, label = [x.to(local_rank) for x in batch]
        with torch.no_grad():
            loss, prob, _ = mymodel(input_id, input_mask, label)
            pred_prob.append(prob.cpu().numpy())
            true_label.append(label.cpu().numpy())
    ...
    if args.do_train:
        if args.task_type == "clone":
            for num, task in enumerate(task_nums):
                ...
                train_dataset = load_clone_data_by_tag(args, tokenizer, "train", tag=args.num)
                eval_dataset = load_clone_data_by_tag(args, tokenizer, "eval", tag=args.num)
                logger.info(f"train_size for {args.num}: {str(len(train_dataset))}")

                if num == 0:
                    mymodel = ClonePPModel(model, args).to(local_rank)
                    ...
                    mymodel = ClonePPModel(model, args, previous_prompt).to(local_rank)
    ...
    if args.do_test:
        ...
        if args.task_type == "clone":
            for num, task in enumerate(range(args.check_order + 1)):
                ...
                args.cl_num = num
                test_dataset = load_clone_data_by_tag(args, tokenizer, "test", tag=num)
                logger.info(f"test_size for {num}: {str(len(test_dataset))}")
                test(args, test_dataset, mymodel, local_rank)
