...
from preprocess_replay import load_clone_replay_data_by_tag, TextDataset
    ...
    if args.do_train:
        if args.task_type == "clone":
            for num, task in enumerate(task_nums):
                ...
                args.cl_num = task_id
                train_dataset = load_clone_replay_data_by_tag(args, tokenizer, "train", tag=task_id)
                eval_dataset = load_clone_replay_data_by_tag(args, tokenizer, "eval", tag=task_id)
    ...
    if args.do_test:
        ...
        if args.task_type == "clone":
            for num, task in enumerate(range(args.check_order + 1)):
                ...
                args.cl_num = task_id
                test_dataset = load_clone_replay_data_by_tag(args, tokenizer, "test", tag=task_id)
