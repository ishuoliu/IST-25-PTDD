...
class ClonePPModel(nn.Module):
    ...
    def init_new_prompt(self, prompt_len):
        N = self.basemodel.embeddings.word_embeddings.weight.shape[0]
        prompt_weigths = []

        for i in range(prompt_len):
            with torch.no_grad():
                j = np.random.randint(N)     # random token
                w = deepcopy(self.basemodel.embeddings.word_embeddings.weight[j].detach().cpu().numpy())
    ...
    def forward(self, input_id=None, input_mask=None, label=None, input_prompt=None, mode="train"):
        if mode == "train":
            ...
            k = input_id.shape[0]
            input_embeds = self.model.embeddings.word_embeddings(input_id)
