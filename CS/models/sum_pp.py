...
class SumPPModel(nn.Module):
    ...
    def get_MLP(self, bottleneck_size):
        if self.args.mlp_type == "none":
            ...
            N = self.basemodel.shared.weight.size(1)
    ...
    def init_new_prompt(self, prompt_len):
        N = self.basemodel.shared.weight.size(0)
        ...
        for i in range(prompt_len):
            with torch.no_grad():
                ...
                w = deepcopy(self.basemodel.encoder.embed_tokens.weight[j].detach().cpu().numpy())
    ...
    def generate_preds(self, source_id, source_mask):
        prompt = self.previous_prompts
