...
from models.sum_prompt import SumPPModel
...
def test(args, test_examples, test_dataset, model, previous_prompt, tokenizer, local_rank):
    ...
    for batch in tqdm(test_dataloader, total=len(test_dataloader)):
        ...
        with torch.no_grad():
            # loss, _ = model(source_id, source_mask, target_id, target_mask)
            preds = model.generate_preds(source_id, source_mask, previous_prompt)
    ...
    if args.do_test:
        ...
        if args.task_type == "sum":
            for num, task in enumerate(range(args.check_order + 1)):
                ...
                test(args, test_examples, test_dataset, mymodel, previous_prompt, tokenizer, local_rank)
