...
class SVDPPModel(nn.Module):
    def __init__(self, basemodel, args, new_prompt=None, previous_prompt=None):
        ...
        if self.args.do_train:
            if self.args.cl_num == 0:
                ...
                initial_tensor = new_prompt.clone()
                self.model.prompt = nn.Parameter(initial_tensor)
        else:
            pass
        ...
        if self.args.do_train and self.args.cl_num == 0:
            self.previous_prompt = torch.zeros([0, self.model.prompt.shape[1]],
                                                requires_grad=False)
        else:
            self.previous_prompt = previous_prompt
        self.previous_prompt = self.previous_prompt.to(args.local_rank)
    ...
    def forward(self, input_id=None, input_mask=None, label=None, input_prompt=None, mode="train"):
        if mode == "train":
            ...
            inputs_embeds = torch.cat((prompt.repeat(k, 1, 1),
                                       self.previous_prompt.repeat(k, 1, 1),
                                       input_embeds),
                                      dim=1)
            full_prefix_len = self.previous_prompt.shape[0] + prompt.shape[0]
            ...
            new_prompt = prompt
            return_prompt = torch.cat((new_prompt, self.previous_prompt), dim=0)

            return loss, prob, new_prompt, return_prompt
