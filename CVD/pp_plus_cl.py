...
from models.svd_pp_plus import SVDPPModel
...
def train(args, train_dataset, eval_dataset, model, local_rank):
    ...
    for idx in range(args.epochs):
        ...
        for step, batch in enumerate(bar):
            ...
            loss, prob, new_prompt, return_prompt = model(input_id, input_mask, label)
            ...
            if (step + 1) % args.save_steps == 0:
                results = evaluate(args, eval_dataset, model, new_prompt, return_prompt, local_rank)
                ...
                if results["eval_f1"] >= best_f1 and dist.get_rank() == 0:
                    ...
                    save_content = {
                        ...
                        "previous_prompt": results["previous_prompt"],
                    }
...
def evaluate(args, eval_dataset, mymodel, new_prompt, previous_prompt, local_rank):
    ...
    for batch in eval_dataloader:
        input_id, input_mask, label = [x.to(local_rank) for x in batch]
        with torch.no_grad():
            loss, prob = mymodel(input_id, input_mask, label, previous_prompt, "eval")
...
def test(args, test_dataset, mymodel, previous_prompt, local_rank):
    ...
    for batch in test_dataloader:
        input_id, input_mask, label = [x.to(local_rank) for x in batch]
        with torch.no_grad():
            loss, prob = mymodel(input_id, input_mask, label, previous_prompt, "test")
            pred_prob.append(prob.cpu().numpy())
            true_label.append(label.cpu().numpy())
...
def main(args):
        ...
        if args.task_type == "svd":
            for num, task in enumerate(task_nums):
                ...
                if num == 0:
                    ...
                    previous_prompt = checkpoint["previous_prompt"]
                    new_prompt = checkpoint["new_prompt"]

                    model.load_state_dict(checkpoint['model_state_dict'])
                    mymodel = SVDPPModel(model, args, new_prompt, previous_prompt).to(local_rank)
    ...
    if args.do_test:
        ...
        checkpoint = torch.load(output_dir, map_location=map_location)
        previous_prompt = checkpoint["previous_prompt"]
        print(previous_prompt.shape)
        model.load_state_dict(checkpoint['model_state_dict'])
        mymodel = SVDPPModel(model, args, previous_prompt=previous_prompt).to(local_rank)
